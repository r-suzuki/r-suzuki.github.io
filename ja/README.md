## Rパッケージ

[`gcor`](https://github.com/r-suzuki/gcor)
: 非線形な相関関係の評価指標

[`dateback`](https://github.com/r-suzuki/dateback)
: Rパッケージ環境の保存と復元

[`pvclust`](https://github.com/shimo-lab/pvclust)
: 階層型クラスター分析の信頼性評価


## 発表資料

### Japan.R 2025
- [パッケージ環境の再現ツールと新しい相関指標](20251206_japanr2025/japanr2025.pdf)（PDF）

### 2025年度 R研究集会
統計数理研究所共同利用研究集会「データ解析環境Rの整備と利用」

#### Rによる非線形相関の評価と再現可能研究
- [Rパッケージ環境の保存と復元](20251123_rjpusers2025/rjpusers2025_dateback.pdf)（PDF）
- [非線形相関の評価指標および推定手法](20251123_rjpusers2025/rjpusers2025_gcor.pdf)（PDF）


### 2025年度 統計関連学会連合大会

#### 非線形な相関関係の評価指標および推定手法

- [概要](20250910_jfssa2025/jfssa2025_abst.pdf)（PDF）
- [スライド資料](20250910_jfssa2025/jfssa2025_slide.pdf)（PDF）


## その他

### R言語 Advent Calendar 2025
- [Rで試す「新しい」相関尺度](20251212_rlang_advent_calendar/r_new_cor.html)

\
[LICENSE](../LICENSE.txt)

---
鈴木 了太（株式会社 ef-prime）\
suzuki@ef-prime.com

All rights reserved.
